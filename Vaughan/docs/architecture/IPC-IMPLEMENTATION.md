...
## Tauri 2.0 Upgrade Caveats

When upgrading to **Tauri 2.0**, two critical things must be maintained for the IPC bridge to work. Both issues will cause the DApp connector to silently drop connections or crash the WebView Javascript if implemented incorrectly.

### 1. Asynchronous `event.listen`
In Tauri v1, `__TAURI__.event.listen` was synchronous. In **Tauri 2.0**, `listen` is asynchronous and returns a `Promise`. 

If you use `listen` inside a raw initializer script without resolving the promise or using `await` inside an `async` IIFE (Immediately Invoked Function Expression), it will crash the injection script with `TypeError: Cannot read properties of undefined (reading 'transformCallback')`. 
Make sure you handle the `listen` Promise:
```javascript
window.__TAURI__.event.listen('wallet_event', (event) => {
    // Handle event...
}).then((unlistenFn) => {
...
### 2. Wildcard Event Permissions (`dapp-access.json`)
By default, Tauri 2.0 limits access to its IPC invoke and listen commands entirely. If you want any arbitrary DApp to connect via the IPC bridge, you must configure the `remote.urls` block in `<capability>.json` to use wildcard matchers rather than a strict whitelist. 

Otherwise, Tauri will block the DApp from registering the `wallet_event` listener with log errors like `/not allowed on window "dapp-xyz"/`. 

Ensure `src-tauri/capabilities/dapp-access.json` looks like this:
```json
"remote": {
  "urls": [
    "http://*/**",
...
```
Validation for connecting to DApps shouldn't be governed here, but rather in the Rust backend logic (via `handle_request_accounts` approvals).
