...
export function ColoredAddress({ address, className }: { address: string; className?: string }) {
    const addrStr = address.startsWith("0x") ? address.slice(2) : address;

    if (addrStr.length < 40) {
        return <span className={`${className} text-[#808080]`}>{address}</span>;
    }
    ...
    return (
        <span className={`${className} font-medium font-mono`}>
...
export function AddressDisplay({ address }: AddressDisplayProps) {
    ...
    return (
        <div
            ...
            title="Copy address"
        >
            <ColoredAddress address={address} className="text-[18px] tracking-wide" />
            {copied ? (
