...
(function() {
  ...
  console.log('[Vaughan] Provider script loaded');
  console.log('[Vaughan] window.__TAURI__ exists:', typeof window.__TAURI__ !== 'undefined');
  ...
  function initializeProvider() {
    ...
    console.log('[Vaughan] window.__TAURI__.core exists:', typeof window.__TAURI__?.core !== 'undefined');
    console.log('[Vaughan] window.__TAURI__.event exists:', typeof window.__TAURI__?.event !== 'undefined');
  ...
  class VaughanProvider extends EventEmitter {
    ...
    async _initialize() {
      try {
        console.log('[Vaughan] Initializing provider...');
        console.log('[Vaughan] isNativeWebview:', this._isNativeWebview);
    ...
    async _sendViaPostMessage(request) {
      console.log('[Vaughan] Sending via postMessage:', request.method, request);
      
      return new Promise((resolve, reject) => {
        // Set up response handler
        const handleResponse = (event) => {
          console.log('[Vaughan] Received message event:', event.data);
          
          // Validate response
          if (!event.data || event.data.type !== 'PROVIDER_RESPONSE') {
            console.log('[Vaughan] Not a PROVIDER_RESPONSE, ignoring');
            return; // Not our response
          }
          
          const response = event.data.data;
          if (response.id !== request.id) {
            console.log('[Vaughan] Response ID mismatch:', response.id, 'vs', request.id);
            return; // Not our response
          }
          
          console.log('[Vaughan] Matched response for request:', request.id);
          ...
          // Handle response
          if (response.error) {
            console.error('[Vaughan] Response error:', response.error);
            reject(new Error(response.error.message || 'Request failed'));
          } else {
            console.log('[Vaughan] Response success:', response.result);
        ...
        console.log('[Vaughan] Listening for response to:', request.id);
        ...
        console.log('[Vaughan] Posting message to parent:', message);
        window.parent.postMessage(message, '*');
        
        // Timeout after 30 seconds
        setTimeout(() => {
          console.warn('[Vaughan] Request timeout for:', request.id);
          window.removeEventListener('message', handleResponse);
  ...
  // Create provider instance (isNativeWebview is determined inside initializeProvider)
  const provider = new VaughanProvider(typeof window.__TAURI__ !== 'undefined');
  console.log('[Vaughan] Provider instance created:', provider);
  ...
  console.log('[Vaughan] window.ethereum assigned:', window.ethereum);
